...
function run(lineHits: number, branchHits: string, rationale = "") {
  const repository = tempDirectory("cue-changed-coverage-");
  const source = "packages/core/src/value.ts";
  mkdirSync(path.join(repository, "packages/core/src"), { recursive: true });
